...
Page({
	...
	addCount: function (e) {
		...
		that.cartToArray(foodId, goodsSpecDetail)

	},
    /**
     * 绑定加数量事件
     */
	addDetailCount: function (e) {
		console.log(e)
		var that = this;
		var foodId = e.currentTarget.dataset.foodId;
		var goodsSpec = e.currentTarget.dataset.goodsSpec;
	...
	cartToArray: function (foodId, goodsSpecDetail) {
		...
		for (var i = 0; i < cartObjects.length; i++) {
			if (cartObjects[i].cartFood.gid == foodId) {
				...
				} else if (cartObjects[i].cartFood.goodsSpec.length > 1) {
					cartObjects[i].goodsSpecDetail = goodsSpecDetail;
